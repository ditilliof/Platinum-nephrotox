...
timestamp_vitro = "2024-08-05_11-21-18"
...
KFCD_max <- sapply(doses_vivo, function(dose) {
  ...
  for(i in 1:num_sim){
    ...
    inistate_vivo <- c(Dose = PK_pars[["Plasma0", "mean"]] * factor_vivo, KidneyPt = 0, AccuPt =
...
DDvivo_max <- sapply(doses_vivo, function(dose) {
  ...
  for(i in 1:num_sim){
    ...
    modelParams <- setModelParameters("Cisplatinvivo", draws_cis, k)
...
DDvitro_max <- sapply(conc_vitro, function(dose) {
  ...
  simulation_results <- matrix(ncol = length(finish_vitro), nrow = num_sim)
   for(i in 1:num_sim){
    ...
    out_vitro <- as.data.frame(ode(y = inistate_vitro, times = finish_vitro, func = get(model_vitro), parms = pars_vitro))
    simulation_output <- as.data.frame(out_vitro)
...
CD_67 <- sapply(doses_vivo, function(dose) {
  ...
  for(i in 1:num_sim){
    ...
    inistate_vivo <- c(
      Dose = PK_pars[["Plasma0", "mean"]] * factor_vivo,
