...
model {
 ...
 // Priors for model parameters
  k_cDD ~ exponential(1);   // Lognormal to reflect positive, skewed values
  degDD ~ exponential(1);   // Lognormal for small decay rates, reflecting skew
  hillDD ~ exponential(1);   // Small positive values, use lognormal
  k_inter ~ exponential(1);  // Interaction rate, lognormal skewed
  maxdeath ~ exponential(1);    // Centered on 1.0 with some variability
  k_hillnec ~ exponential(1);   // Centered on 5.0 to reflect central tendency
  p ~ beta(2, 2);                  // Beta distribution for bounded parameter (0, 1)
  h ~ normal(20, 5);               // High centrality with narrower uncertainty
  // Priors for degrees of freedom (Student's t-distribution) v 
